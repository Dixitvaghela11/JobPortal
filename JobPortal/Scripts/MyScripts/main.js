...
function validatePhone(input) {
    ...
    if (/^\d{10}$/.test(phoneNumber)) {
        errorSpan.textContent = '';
        $.ajax({
            url: "/Home/CheckPhoneNumber/?number=" + phoneNumber,
            type: "GET",
            success: function (data, textStatus, xhr) {
                if (xhr.status === 202) {
                    $("#error-phone").text("Number is already registered").removeClass("success-msg").addClass("error-msg");
                }
                return false;
            }
        });
...
function validateCompanyName(input) {
    ...
    function validateImage(input) {
        ...
        if (!allowedExtensions.includes('.' + fileExtension)) {
            errorSpan.textContent = 'Invalid file format. Allowed formats: .png, .jpg, jpeg, .bmp, .webp ';
            input.value = '';
            return false;
        } else {
            errorSpan.textContent = '';
    ...
    function checkPhone(phoneNumber) {
        var validityPhone = document.getElementById('error-phone');
        if (validityPhone.textContent != '')
            return false;
        else {
            if (!/^\d{10}$/.test(phoneNumber.value)) {
                validityPhone.textContent = 'Please enter a valid 10-digit phone number.';
                return false;
            }
            return true;
        }
...
function checkUsername(username) {
    var validityUsername = document.getElementById('error-username');
    if (validityUsername.textContent == "Username is available") {
        return true;
    }
    else if (validityUsername.textContent != '')
        return false;
    else {
        if (username.length < 4) {
            validityUsername.textContent = 'Username is required';
            return false;
        }
    }
}
    function validateRegisterForm() {
        ...
        const isUsernameValid = checkUsername(document.getElementById('username').value);
        ...
        const isPhoneValid = checkPhone(document.getElementById('PhoneNumber'));
...
function validateEmployerRegisterForm() {
    ...
    const isPasswordValid = validatePass(document.getElementById('Password'));
    const isCPasswordValid = validateCpass(document.getElementById('ConfirmPassword'));
    const isPhoneValid = checkPhone(document.getElementById('ContactPhone'));
    const isEmailValid = validateEmail(document.getElementById('Email'));
    ...
    const isDesignation = validateDesignation(document.getElementById('Designation'))
    if (
        isCompanyLogo &&
        isUsernameValid &&
        isPasswordValid &&
...
function validateDescription() {
    ...
    if (wordCount < 0) {
        errorSpan.textContent = "Description must contain at least 0 words.";
        return false;
    }
    return true;
}
